...
function SignUpPage() {
  ...
  const handleRequestOtp = async (e) => {
    ...
    await axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/auth/signup/send-otp`, {
        email: email.toLowerCase(),
      })
      .then((response) => {
  ...
  return (
    <>
      {" "}
      {isMobile && (
        <header className="fixed inset-0  w-full bg-black text-white text-center py-4">
          <div className="flex justify-between mx-2 md:mx-4 lg:mx-16">
            <Image src="/exe.png" alt="logo" width={25} height={25} />
            <h1 className="text-2xl mx-auto text-center justify-center  font-bold">
              MemeWebsite
            </h1>
            <Image src="/exe.png" alt="logo" width={25} height={25} />
          </div>
