...
function MemeSection({ showmeme, memes, user, handleUpvote, handleComment }) {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/user/auth`, {
        headers: {
  ...
  const fetchComments = async (id) => {
    ...
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/post/comments/get/${id}`)
      .then((response) => {
  ...
  return (
    <div className="p-4 bg-black">
      {memes.map((meme) => (
        <div
          key={meme._id}
          className="lg:max-w-xl md:max-w-md min-[1160px]:mx-auto md:ml-auto md:mr-20 min-[840px]:mr-32 max-w-sm mx-auto  shadow-md rounded-lg px-4 py-6 mb-4 border bg-black border-gray-800"
        >
          ...
          <span className="text-gray-500 text-xs mb-5 ml-10">
            {format(new Date(meme.createdAt), "dd MMM yyyy")}
          </span>
          <h1 className="text-xl my-2 font-semibold text-white overflow-x-auto no-scrollbar">
            {meme.Title}
          </h1>
          {meme.Image ? (
            <img src={meme.Image} alt="Meme" className="w-full h-auto mb-2" />
          ) : (
            <></>
          )}
          <p className="text-white my-3 overflow-x-auto no-scrollbar">
            {meme.Caption}
          </p>
          <div className="flex justify-start mt-8">
            <div className="md:w-1/6 w-1/2 flex flex-col justify-between md:justify-start items-center">
              {!meme.Upvotes.includes(user) ? (
                <Upvoteicon
                  onClick={() => handleUpvote(meme._id, false)}
                  style={{
                    fill: "white",
                    cursor: "pointer",
                    width: 30,
                ...
                <Upvotedicon
                  onClick={() => handleUpvote(meme._id, true)}
                  style={{
                    fill: "#c084fc",
                    cursor: "pointer",
                    width: 30,
              ...
              <span className="text-white text-xs mt-2">
                {meme.Upvotes.length}{" "}
                {meme.Upvotes.length === 1 ? "Upvote" : "Upvotes"}
              </span>
            </div>
            <div className="md:w-1/6 w-1/2 flex flex-col justify-between md:justify-start items-center">
              <Chat
              ...
              >
                ...
                <div className="flex flex-col max-h-44 no-scrollbar overflow-y-auto gap-2 w-full">
                  {isCommentLoading ? (
                    ...
                    Comments.map((comment) => (
                      <div
                        key={comment._id}
                        className="border-gray-700 border p-2 rounded-lg flex items-center gap-2"
                      >
                        <img
                ...
                <div className="flex justify-between items-center w-full mt-4">
                  <input
                    ...
                    className="bg-gray-800 text-white p-2 rounded-lg w-3/4"
                    onChange={(e) => {
                  ...
                  <button
                    onClick={() => {
                      handleComment(meme._id, comment, () => {
                        fetchComments(meme._id);
                        setComment("");
                      });
                    }}
                    className="bg-[#c084fc] text-white text-lg p-2 rounded-lg w-1/4 ml-2"
                  >
                    Post
                  </button>
                </div>
